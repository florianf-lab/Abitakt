...
**Kalender-Export (6.12).** `buildIcs()` erzeugt eine RFC-5545-konforme .ics-Datei aus dem *frisch gerechneten* Tagesplan (`Coach.plan()`), allen offenen Terminen (Ganztagestermin mit Erinnerung einen Tag vorher) und allen fälligen Aufgaben. `icsEscape` maskiert `\ ; , ` und Zeilenumbrüche, `icsFold` faltet auf 75 Oktette, Zeilenenden sind CRLF. `ACT.exportIcs` lädt herunter; ohne Inhalte gibt es einen ehrlichen Hinweis statt einer leeren Datei. Knopf über `icsButton()` im Tagesplan. Es wird nichts hochgeladen.

**Stolpersteine (6.11).** Jede falsch beantwortete Übungsaufgabe landet in `S.misses` (`{id, topicId, q, opts, a, chosen, hint, why, n, streak, at}`, höchstens 150). `recordAnswer(topicId, q, chosen, correct)` hängt in `ACT.ansPractice` und im Drill; zwei richtige Antworten in Folge (`MISS_SOLVE_AT`) löschen den Eintrag, eine falsche setzt die Serie zurück. Ansicht `scrMisses()` (Reiter „Fehler“, `VIEW = "miss"`), Drill `startDrill(scope)` / `renderDrill()` / `finishDrill()` mit höchstens 12 Aufgaben. **Der Drill verändert die Engine-Mastery bewusst nicht** – er zählt als Lernzeit (`Coach.record({kind:"review"})`) und frischt `S.lastPractice` auf. Dashboard-Karte: `missCard()`.
